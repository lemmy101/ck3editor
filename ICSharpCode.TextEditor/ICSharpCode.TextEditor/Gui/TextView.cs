...
namespace ICSharpCode.TextEditor
{
	...
	public class TextView : AbstractMargin, IDisposable
	{
		...
		void PaintDocumentLine(Graphics g, int lineNumber, Rectangle lineRectangle)
		{
			...
			if (selectLine == null)
            {
             //   selectLine = new SolidBrush(Color.LightBlue);

            }
            if (DebuggerData.selectedLine == lineNumber)
            {
         //       backgroundBrush = selectLine;
           //     bgColorBrush = selectLine;
...
//			bgColorBrush.Dispose();
		...
		Brush GetBgColorBrush(int lineNumber)
		{
            ...
            if (DebuggerData.selectedLine == lineNumber)
            {
                return BrushRegistry.GetBrush(textArea.Document.HighlightingStrategy.GetColorFor("CaretLine").BackgroundColor);
		...
		int PaintLinePart(Graphics g, int lineNumber, int startColumn, int endColumn, Rectangle lineRectangle, int physicalXPos)
		{
			...
			repeatDrawCurrentWord:
                ...
                if (DebuggerData.selectedLine == lineNumber)
                {
                    wordBackBrush = BrushRegistry.GetBrush(textArea.Document.HighlightingStrategy.GetColorFor("CaretLine").BackgroundColor);
